#####################################################################################
# reads pkl file of 2015NBCC hazard model and outputs an OpenQuake source file with
...
outxml = path.join('temp_source_files', ''.join((outbase,'_collapsed_rates_FF.xml')))
...
outxml = path.join('temp_source_files', ''.join((outbase, '_source_model_logic_tree_FF.xml')))
f = open(outxml,'w')
f.write(newxml)
f.close()
