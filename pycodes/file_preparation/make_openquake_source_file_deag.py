...
for i, bl in enumerate(betalist):
    for j, ml in enumerate(maglist):
                ...
                if isinf(log10(m['src_N0'][i])) == False:
                    ...
                    if m['fault_dip'][0] >= 0: # catches all faults
                        ...
                        if m['src_beta'][i] > -99:
                            ...
                            octxt = make_incremental_occurrence_text(m, effN0, binwid)
        ...
        outxml = path.join('temp_source_files', '_'.join((sitename,outbase,bl,ml,'deag.xml')))
        f = open(outxml,'w')
        f.write(newxml)
        f.close() 
        
        srcxmls.append('_'.join((sitename,outbase,bl,ml,'deag.xml')))
